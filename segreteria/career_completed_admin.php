...
<html lang="en">
  <head>
    ...
    <title>servizio carriere complete segreteria</title>
    ...
    <body>
        ...
        <div class="container pt-1 pb-3 mt-5 border">
          <h3 class="text-center">Carriere complete degli studenti</h3>
          <br>
        <?php
        ...
        ?>
          ...
          <th style="text-align: center;"><a href=<?php echo ("{$_SERVER['PHP_SELF']}?matricola={$s['matricola']}") ?>
        ...
        <?php
            ...
            if (isset($_GET['matricola'])) {
                ...
                if ($tipo_studente == 'rimosso') {
                    $carriera = get_career_completed_removed_student($_GET['matricola']);
                    $studente = get_hist_student_data_from_id($_GET['matricola']);
                } else if ($tipo_studente == 'attivo') {
                    $carriera = get_career_completed($_GET['matricola']);
                    $studente = get_student_data_id($_GET['matricola']);
                }

                    ?>
            <h4 class="text-center"><?php echo ("{$studente['nome']} {$studente['cognome']}"); ?></h4>
          <br>
          <table class="table table-bordered table-hover">
            <thead class="thead-light">
                <tr>
